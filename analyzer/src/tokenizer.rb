...
SYMBOLS = ["(",")","[","]","{","}",".",",",";","+","-","*","/","&","|","<",">","=","~"]
IDENTIFIERS = ["int","char","boolean","Array"]
CONVERSIONS = [{s: "<", t: "&lt;"},{s: ">",t: "&gt;"},{s:"&", t: "&amp;"}, {s: "\"", t: '&quot;'}]
@tokenizer = $PROGRAM_NAME.include?('tokenizer.rb')
...
def writeXML(obj)
	ret = @tokenizer ? 'T.xml' : '.xml'
	output = File.new(File.expand_path(obj[:file]+ret),'w')
	output.puts obj[:lines]
end

if @tokenizer
	hidden = handleInput()
	hidden.map{|x| writeXML(x)}
end
